<template>
  <div class="limit-width" style="margin-top:20px;">
    <Form ref="TableInfo" :model="TableInfo" :rules="rules" :label-width="100">
      ...
      <Form-item label="价格" prop="productPrice">
        <Input
          type="number"
          v-model="TableInfo.productPrice"
          placeholder="请输入价格"
          style="width:300px;"
        />
      </Form-item>
      <Form-item label="产品信息" prop="productInformation">
        <Input
          type="string"
          v-model="TableInfo.productInformation"
          placeholder="请输入产品信息"
          style="width:300px;"
        />
      </Form-item>
      <Form-item label="产品数量" prop="number">
        <Input type="string" v-model="TableInfo.number" placeholder="请输入产品数量" style="width:300px;" />
      </Form-item>
...
<script>
import customerservie from "@/api/Customerinfo";
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
      ...
      customerservie.update(formdata).then(res => {
        if (res.data.code === 200) {
          this.$Message.success("success");
          this.$router.push({ path: "/customer/table/shipping/list" });
        }
      });
    }
    // onSubmit () {
